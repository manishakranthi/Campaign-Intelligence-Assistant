import { Card, CardHeading, formatCurrency, formatPercent, PlatformBadge } from "./primitives";
      ...
      <CardHeading
        ...
        title={
          <>
            ...
            <span className="ml-1.5 rounded-full bg-sky-100 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-sky-800 dark:bg-sky-950 dark:text-sky-300">
              Pre-launch estimate
            </span>
          </>
        }
...
export function BudgetReallocationCard({
  reallocation,
  bare = false,
}: {
  reallocation: BudgetReallocationResult;
  bare?: boolean;
}) {
  const content = (
    <>
      {!bare && <CardHeading campaignId={reallocation.campaignId} title="Budget Reallocation" />}
      <div className="mb-3 flex flex-col gap-1">
        {reallocation.ranked.map((r, i) => (
          <div key={r.platform} className="flex items-center justify-between rounded-lg border border-zinc-200 px-3 py-1.5 text-sm dark:border-zinc-800">
